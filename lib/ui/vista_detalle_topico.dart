...
class VistaDetalleTopico extends StatefulWidget {
  ...
  const VistaDetalleTopico({super.key, required this.topico});
...
class _VistaDetalleTopicoState extends State<VistaDetalleTopico> {
  ...
  Widget build(BuildContext context) {
    ...
    final canEdit = canEditTopico(
      currentUser: _currentUser,
      topico: _topicoActual,
    );
    ...
    return BlocProvider(
      create: (_) =>
          ComentarioBloc(ComentarioService())
            ..add(LoadRespuestas(_topicoActual.id!)),
              ...
              if (state is ComentarioOperationSuccess) {
                ...
                if (_topicoActual.id != null) {
                  context.read<ComentarioBloc>().add(
                    LoadRespuestas(_topicoActual.id!),
                  );
                }
            ...
            listener: (context, state) {
              if (state is TopicoActionSuccess) {
                ...
                if (_pendingDeleteId != null &&
                    state.affectedTopicoId == _pendingDeleteId) {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Imagen del tópico (thumbnail expandible)
                        if (_topicoActual.fotoUrl != null &&
                            _topicoActual.fotoUrl!.isNotEmpty) ...[
                          const SizedBox(height: 16),
                          GestureDetector(
                            onTap: () =>
                                _showFullImage(context, _topicoActual.fotoUrl!),
                            child: Container(
                              height: 180,
                        ...
                        const SizedBox(
                          height: 100,
                        ), // Espacio para el input de comentarios
                      ],
                    ),
                  ),
                ),
    ...
    return StreamBuilder<int>(
      ...
      builder: (context, snapshot) {
        ...
        return FutureBuilder<bool>(
          future: _likesService.hasUserLiked(
            _topicoActual.id!,
            _currentUserId!,
          ),
  ...
  Widget _buildCommentCard(Comentario comentario) {
    ...
    final isLiked = comentario.id != null
        ? (_likedComentarios[comentario.id] ?? false)
        : false;
    final likeCount = comentario.id != null
        ? (_likeCounts[comentario.id] ?? 0)
        : 0;
